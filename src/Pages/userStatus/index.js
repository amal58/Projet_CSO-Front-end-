                ...
                <Row gutter={[20, 20]}>
                  {filteredUsers.map((user) => (
                    <Col key={user._id} xs={24} sm={12} md={8} lg={6}>
                      <Card title={`${user.nom} ${user.prenom}`} bordered={false} className="user-card" style={{ minHeight: "250px" }}>
                        <Text strong>Tessiture Vocale: <span style={{ color: 'green' }}>{user.tessitureVocale}</span></Text>
                        <br />
                        <Text type="secondary">Status:</Text>
                        {user.historiqueActivite.map((activity, index) => (
                          <div key={index}>
                            <Text><b>Saison:</b> {activity.Saison}, <b>Statut:</b> {activity.status}</Text>
                          </div>
                        ))}
                      </Card>
                    </Col>
