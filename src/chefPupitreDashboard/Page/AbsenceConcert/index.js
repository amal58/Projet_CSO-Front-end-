...
function Choristee() {
  ...
  async function getConcerts(choristeId) {
    try {
      let result = await fetch(`http://localhost:3000/concerts/getall/${choristeId}`, {
        method: "GET",
      });
      result = await result.json();
      const concerts = result.concerts; // Assurez-vous de récupérer les concerts depuis la réponse JSON
      const filteredConcerts = concerts.map(concert => ({ id: concert._id, title: concert.titre }));
  
      setConcertsList(filteredConcerts); // Mettez à jour la liste des concerts filtrés
    } catch (error) {
      console.error("Erreur lors de la récupération des données :", error);
    }
  }
  
  const handleCheckChange = (auditionId) => {
    setOpenPopup(true);
    setSelectedChoriste(auditionId);
    getConcerts(auditionId);
  };
  ...
  const handleMarquerPresence = async () => {
    try {
      const response = await fetch(`http://localhost:3000/apii/concert/marquerManuellement_concert/${id}`, {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        swal({
          title: "Presence Marqueé avec succès",
          icon: "success",
          button: "Valider",
        });
        close();
        ...
        <div style={{ borderRadius: "50px" }}>
          <section className="py-1 bg-blueGray-50 pr-10 lg:pr-0">
            <div className="w-full xl:w-11/12 mb-12 xl:mb-0 px-4 mx-5 mt-12 mr-40 lg:mr-0" style={{ borderRadius: "50px" }}>
              ...
              <h4 className="text-3xl my-2">  &nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;<u style={{ color: "gray" }}>Marquer Presence à un concert</u></h4>
              <br />
